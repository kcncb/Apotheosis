...
//
//   c++ -std=c++20 -I Apotheosis Apotheosis/runtime/tests/latency_probe_test.cpp -o lp_test
//
...
static void runFrame(int capWaitUs, int inferUs, int pubToAimUs)
{
    ...
    lat::markAimConsume(cap, pub, 0.5);
}

int main()
{
    printf("=== latency_probe 自测 ===\n\n");
    ...
    printf("\n[2] detector 跟不上时统计丢帧\n");
    {
        ...
        CHECK(lat::snapshot().dropped_capture == 0, "连续取帧不记账为丢帧");
        ...
        const auto s = lat::snapshot();
        CHECK(s.dropped_capture == 3, "跳过 3 帧被计入 dropped_capture");
    ...
    printf("\n[3] 无采集戳的消费不应污染统计\n");
    {
        ...
        lat::markAimConsume(0, lat::nowNs(), 0.0);
        lat::markAimConsume(0, lat::nowNs(), 0.0);

        const auto s = lat::snapshot();
        CHECK(s.stages[lat::kTotal].n == before, "无戳消费不进 total 样本");
        CHECK(s.stale_consumes == 2, "无戳消费计入 stale_consumes");
    }
    ...
    printf("\n[4] reset 清空\n");
    {
        lat::reset();
        const auto s = lat::snapshot();
        CHECK(s.frames_consumed == 0 && s.dropped_capture == 0, "计数器归零");
    ...
    printf("\n[5] 开关\n");
    {
        ...
        CHECK(!lat::enabled(), "setEnabled(false) 生效");
        CHECK(lat::formatLinesAscii(true).size() == 1, "关闭时只输出一行提示");
        lat::setEnabled(true);
        CHECK(lat::enabled(), "重新开启");
    }
    ...
    printf("\n[6] 输出格式\n");
    {
        ...
        CHECK(!ascii.empty(), "ASCII 版非空");
        bool all_ascii = true;
        for (const auto& line : ascii)
            for (unsigned char ch : line)
                if (ch > 0x7E || (ch < 0x20 && ch != 0x09)) all_ascii = false;
        CHECK(all_ascii, "ASCII 版不含非 ASCII 字节 (OpenCV putText 只能画 ASCII)");

        const std::string cn = lat::formatLines(true);
        CHECK(cn.find("总延迟") != std::string::npos, "中文版含 总延迟 字样");
        printf("      --- ascii ---\n");
    ...
    printf("\n[7] 并发冒烟 (采集线程 + 消费线程)\n");
    {
        ...
        std::thread consumer([&] {
            while (run.load()) {
                ...
                if (cap != 0) lat::markAimConsume(cap, lat::nowNs(), 0.3);
                spin_us(300);
            }
        });
        ...
        const auto s = lat::snapshot();
        CHECK(s.frames_consumed > 0, "并发下仍结算到帧");
        CHECK(s.stages[lat::kTotal].ema_ms >= 0.0, "并发下统计无异常值");
