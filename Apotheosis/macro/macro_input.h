...
// Mouse dispatch routes only through MAKCU or MAKCUNEW. Keyboard macros use
// keybd_event because both supported hardware backends are mouse-only.
...
// a macro script can't corrupt the MAKCU serial framing.

// Relative cursor move in pixels.
void dispatch_move(int dx, int dy);

// Move to an absolute screen coordinate by reading the current cursor and
// sending a relative delta through the selected MAKCU backend.
...
// vk: Win32 VK_* code. MAKCU devices are mouse-only, so this uses keybd_event.
