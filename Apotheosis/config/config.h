...
struct HotkeyProfile
{
    ...
    //   **窗口 > 0 时引擎会自动把计数域那项(beta)置 0**(见 boss_aim.cpp 的
    //   am_inflight 判据)—— 两处同开 = 同一批在途指令被扣两次 = 过补偿 = 正反馈发散。
    //   ★ 反过来也成立: 窗口 = 0(默认)时 AM 这条链没在跑, 计数域那项就是【唯一的】
    //     在途补偿, 不许顺手关掉 —— beta = 0 实测是"拆掉主刹车"(60fps 发散)。
