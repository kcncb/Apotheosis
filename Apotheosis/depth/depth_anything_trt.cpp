...
namespace depth_anything
{
    namespace
    {
        ...
        {
#ifdef _WIN32
            const std::string leftStr = left.u8string();
            const std::string rightStr = right.u8string();
        ...
        bool ResolveDepthModelPath(const std::string& modelPath, std::string& resolvedPath, std::string& error)
        {
            ...
            std::filesystem::path requested(std::filesystem::u8path(modelPath));
            ...
            {
                error = "Depth model directory not found: " + baseAbs.u8string();
                return false;
            }
            ...
            resolvedPath = resolvedAbs.u8string();
            return true;
        }
    ...
    bool DepthAnythingTrt::initialize(const std::string& modelPath, nvinfer1::ILogger& logger)
    {
        ...
        if (!std::filesystem::exists(std::filesystem::u8path(resolvedPath)))
        {
