...
// ── 非原生: 本项目为"当前上下文"补充的输入 ──────────────────────────────────
//
// 刻意与原生的 PidfInputExact 分开, 这样"哪些是复刻二进制来的、哪些是我们为
// 自己的链路加的"一眼可分, 将来要对照反汇编也不会混淆。
struct PidfContextInput {
    // 端到端提前时间(秒)。
    //
    // 由 latency_probe 实测的 total(T3-T0, 即"像素进入本进程 -> 控制环消费到它")
    // 提供, 每帧刷新 —— 不写死常数, 所以数据集/分辨率/模型/帧率一变, 提前量会
    // 自己跟着变。0 表示关闭提前量, 行为与原生实现完全一致(便于 A/B)。
    //
    // 为什么需要它: 原设计里补这段滞后的是被单独一级 QX 弹道预测, 而当前集成把
    // QX 关掉了(qx_stage_selected=false), 于是这一级必须由 PIDF 的前馈自己承担。
