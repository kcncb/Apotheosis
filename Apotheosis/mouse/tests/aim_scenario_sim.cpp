...
//
//   7b) 【延迟 <-> 可达性能】的量化(同一控制器, 只改链路延迟与 lr):
//         lr      0帧延迟    1帧     2帧     3帧
//         0.05      9.23    12.47   15.19   17.77
//         0.10      6.53    10.57   19.00   24.15
//         0.20      5.53    16.65   53.77   64.14
//         0.30      5.53    33.22   92.10   99.13
//         完美速度  2.71     5.66    5.89     -
//      每减少一帧延迟, 就多解放一档估计器速度: 0 帧时 lr=0.2 能跑到 5.53(几乎追平
//      oracle), 而 2 帧以上只有 lr=0.05 可用。也就是说【当前 3 帧延迟下 lr=0.05 已经
//      是这个回路允许的最优】, 剩余 3 倍空间被延迟锁住 —— 降延迟比继续调 PID 划算。
//      (注: 这里的"帧"是 120Hz 下的一帧 = 8.33ms; 实机 total 延迟换算时按此。)
