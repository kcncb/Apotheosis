...
//      或回路增益结构。
//
//   8d) 【重要】被控对象增益(游戏内灵敏度)是本算法最大的隐藏依赖。
//       本应用没有灵敏度/缩放字段, 位移直接下发(1 计数 = 1 屏像素是隐含假设),
//       而误差是【屏幕像素】—— 两者之间那个系数就是被控对象增益 g。实测
//       (argv[15] 可设, 3 帧延迟, kp=2.0 kd=0.05 kf=1 lr=0.08):
//           g       0.5     0.7    1.0    1.3     2.0      3.0
//           分数   21.90   14.94  12.07  13.17  162.77   发散
//       即 g≈1 时最优, g>1.5 后迅速恶化, g≥2 会【发散】(准星自己乱飞)。
//       稳态其实与 g 无关(前馈会学到被 g_est/g 缩放的估计, 输出除法正好抵消),
//       发散纯粹因为【回路增益被放大】(有效增益 ≈ kp*g), 相位裕度不够。
//       kp x g 前沿:
//           kp      g=1.0    g=1.5    g=2.0    g=2.5
//           2.0     12.07    20.36   162.77   1.9e5
//           1.4     13.71    13.75    73.79   495.6
//           1.0     15.62    13.41    50.11   277.3
//           0.5     20.93    15.25    18.31   157.7
//       所以部署前必须校准: 关闭辅助, 让鼠标走 N 个计数, 量目标在检测图上移动了
//       多少像素; 调游戏内灵敏度使 1 计数 ≈ 1 像素。否则参数定标整体偏移。
//       (试过在线估计 g: 归一化 LMS 用观测器创新做回归量会发散 —— 因为指令本身
//        含前馈输出、与误差相关, 回归量被回路污染。未采用。)
//   8e) 【重要】移动限幅是硬约束, 对追踪的影响比任何算法参数都直接。
//       能追上的最高目标速度 = 限幅 x 检测帧率, 所以限幅不够时目标根本追不上,
//       这不是控制器能补的。实测(argv[5], 3 帧延迟, 当前算法):
//           限幅      0(关)     3       5       8      15      30
//           综合分    12.07   80.15   38.72   17.21   13.27   12.10
//           喷气     9.79   325.96  190.50   23.09    9.79    9.79
//           滑铲     2.33   157.43    7.92    2.33    2.33    2.33
//       需要的下限(120Hz): 250px/s 需 2.1/帧, 750px/s 需 6.3/帧, 950px/s 需 7.9/帧;
//       60Hz 检测时全部翻倍。建议保持 0, 要限就给足(120Hz 下 >=15)。
//       (注: 限幅顶住时【不会失控滑行】是 54c817a 那个 residual 加界修复的功劳 ——
//        无界回灌时实测会冲过头 328~531px 且永不归零。)
//   8f) 【调参方法学】⚠️ 必须扫【每一个】延迟档, 尤其别漏 4 帧 —— 这套回路存在
//       与 4 帧延迟共振的不稳定点, 只在 4 帧显现、而 3/5/6 帧完全正常:
//         · kd = 0.08: 2/3 帧正常(9.62/11.70), 4 帧发散(18347), 5 帧 9.7e5
//         · kDerivativeTauSec = 0.005: 2/3/6 帧正常, 4 帧发散(1124)
//       后果: 只扫部分延迟会得出"这个值更优"的错误结论。本会话两次踩到
//       (tau 那次还提交了一版)。
//   8g) 【联合寻优复核】逐个参数调可能停在次优角落, 所以做了一次三参数联合网格
//       (kp x kd x lr, 3 帧档, 关掉定档看真实最优):
//           最优 kp=2.4 kd=0.08 lr=0.08 -> 11.03 (当前 2.0/0.05/0.08 -> 12.07)
//       但把它放到其他延迟档就崩: kd=0.08 在 4 帧发散, kp=2.4 在 4/5 帧 15.71/86.22
//       (当前设定 14.97/32.52)。即低延迟档的最优【不是稳健最优】, 现有的延迟定档
//       恰好把它挡在外面 —— 定档在这里起了关键作用, 不只是"保守兜底"。
//       逐档复核(无定档, kp/kd):  2帧    3帧    4帧     5帧
//           2.0/0.05(现在)      9.88  12.07  14.97   32.52
//           2.4/0.05            9.18  11.74  15.71   86.22
//           2.0/0.08            9.62  11.70  18347   9.7e5
//   8h) 【否定结果】自适应半径依赖目标框尺寸(用框宽算 X、框高算 Y, 人形框 20x48
//       使两轴门控尺度差 2.4 倍)这一设计: 实测把框高从 32 变到 80, 各场景分数
//       【完全不变】(段差 <0.001) —— 因为门控尺度取 3.0 后高斯权重在常见误差下
//       几乎恒为 1, 框尺寸敏感性被掩盖。即当前参数下这个不对称无实际影响。
//   8i) 【理论最优对照】把被控对象写成状态空间(状态 = [误差, 目标速度, 待生效指令]),
//       用 LQR 解出该模型下的最优线性律, 再放进本测试台实测:
//         LQR 给出的律:  u_k = k_e*(e - u_{k-1}) + k_v*v      (k_e≈0.99, k_v≈2*dt)
//         无延迟时它退化为死拍解 u = e + v*dt —— 验证了模型构造正确。
//       即最优律里【有一项反馈"已下发但还没生效的指令"】(u_{k-1}, 系数为负:
//       已经在路上的位移要从"还差多少"里扣掉), 而现有控制器没有这一项;
//       且误差系数(≈1.0)远大于现有(0.1*kp=0.2), 前馈系数是最优的 2 倍。
//       但实测放进本测试台: k_e=1.0/k_v=1 时 2/3/4 帧 = 9.46/12.15/16.62,
//       对现有 9.88/12.07/15.36 —— 只在 2 帧略好, 4 帧更差, 6 帧发散;
//       单独把 u_{k-1} 项加到现有律上更糟(alpha=0.2 时 3 帧 15.61)。
//       原因: LQR 假设速度已知、无噪声、被控对象线性, 这三条在本系统都不成立;
//       现有结构的非线性(门控/反向压制)与延迟定档在真实条件下更稳。
//       结论: 【该简化模型下的理论最优线性律打不过现有结构】—— 但这不等于
//       "现有结构全局最优"(LQR 只是受限对照, 且本测试台的保真度有限)。
...
struct Params {
    ...
    // argv[16]/argv[17] 可覆盖, 用于测自适应半径依赖框尺寸这一设计的敏感性
};
...
Metrics runScenario(const Scenario& sc, const Params& p, const Env& e, int argc, char** argv)
{
    ...
    for (int f = 0; f < frames; ++f)
    {
        ...
        // argv[15] = 被控对象增益(游戏内灵敏度): 鼠标移动 1 单位的屏幕位移。
        // 1.0 = 假设的 1:1; 非 1 时控制器的增益标定会整体偏移。
        const double plant_gain = (argc > 15) ? std::atof(argv[15]) : 1.0;
        err += (enemy_v + self_v) * e.dt - plant_gain * applied;
...
{
    // 参数位: kp kd kf lr 限幅 检测延迟 鼠标延迟 [假定测量延迟 [假定指令延迟 [kf_y [lr_y [噪声px [dt抖动 [检测帧率 [被控对象增益]]]]]]]]
    Params p;
    Env e;
