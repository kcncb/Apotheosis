...
static std::vector<std::string> GetModelFilesByExtInDir(const std::string& dir, const std::vector<std::string>& exts)
{
    ...
    std::error_code ec;
    std::filesystem::directory_iterator it(std::filesystem::path(Utf8ToWide(dir)), ec);
    if (ec)
    {
        return files;
    }
...
{
    return GetModelFilesByExt({ ".engine", ".onnx", ".oliver" });
}
...
std::vector<std::string> getAvailableModels()
{
    ...
    if (config.backend == "TRT")
    {
        ...
        {
            engineModels.insert(std::filesystem::path(Utf8ToWide(file)).stem().u8string());
        }
        ...
        {
            std::string modelName = std::filesystem::path(Utf8ToWide(file)).stem().u8string();
